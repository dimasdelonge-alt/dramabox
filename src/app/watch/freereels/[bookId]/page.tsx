...
export default function FreeReelsWatchPage() {
  ...
  // Sync time from HLS events (more frequent updates for smoother sync)
  useEffect(() => {
    // We can hook into the existing hlsRef or just rely on video.timeUpdate which is usually enough for subtitles (250ms)
    // But for perfect sync we can add a listener if HLS is active
    if (hlsRef.current) {
      const onFragChanged = (_: any, data: any) => {
        if (videoRef.current) setCurrentTime(videoRef.current.currentTime);
      };
      hlsRef.current.on(Hls.Events.FRAG_CHANGED, onFragChanged);
      return () => {
        hlsRef.current?.off(Hls.Events.FRAG_CHANGED, onFragChanged);
      };
    }
  }, [hlsRef.current]);
      ...
      <div className="absolute top-0 left-0 right-0 z-40 h-16 pointer-events-none">
        ...
        <div className="relative z-10 flex items-center justify-between h-full px-4 max-w-7xl mx-auto pointer-events-auto">
          ...
          >
            ...
            <span className="text-primary font-bold hidden sm:inline shadow-black drop-shadow-md">NunoDrama</span>
          </Link>
        ...
        <div className="relative w-full h-full flex items-center justify-center">
          {currentVideoUrl ? (
            <>
              ...
              <SubtitleOverlay
                url={(currentEpisodeData?.originalAudioLanguage !== 'id-ID') ? proxiedSubtitleUrl : ""}
