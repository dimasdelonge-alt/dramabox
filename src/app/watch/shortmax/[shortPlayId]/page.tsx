...
export default function ShortMaxWatchPage() {
    ...
    useEffect(() => {
        ...
        const isHlsUrl = videoUrl.includes('.m3u8') || videoUrl.includes('/api/shortmax/hls');
                ...
                <div className="relative z-10 flex items-center justify-between h-full px-4 max-w-7xl mx-auto pointer-events-auto">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div className="relative">
                            ...
                            {showQualityMenu && (
                                <>
                                    <div className="fixed inset-0 z-[90]" onClick={() => setShowQualityMenu(false)} />
                                    <div className="absolute right-0 top-full mt-1 z-[100] bg-zinc-800 rounded-lg shadow-xl border border-white/10 overflow-hidden min-w-[100px]">
                                        {qualityOptions.map((option) => (
                                            <button
                                                key={option.key}
                                                onClick={() => { setSelectedQuality(option.key); setShowQualityMenu(false); }}
                                                className={cn(
                                                    "w-full px-4 py-2 text-sm text-left hover:bg-white/10 transition-colors",
                                                    selectedQuality === option.key ? "text-primary font-semibold" : "text-white/80"
                                                )}
                                            >
                                                {option.label}
                                            </button>
