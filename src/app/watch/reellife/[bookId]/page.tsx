...
import { useReelLifeDetail, useReelLifeEpisodes, useReelLifeStream } from "@/hooks/useReelLife";
...
export default function ReelLifeWatchPage() {
    ...
    const { data: detail, isLoading: loadingDetail } = useReelLifeDetail(bookId);
    const { data: episodes, isLoading: loadingEpisodes } = useReelLifeEpisodes(bookId);

    const currentEpisode = episodes?.[currentEpisodeIndex];
    const { data: streamData, isLoading: loadingStream } = useReelLifeStream(
        bookId,
    ...
    useEffect(() => {
        if (streamData?.videoInfoList?.length > 0 && !selectedQuality) {
            const q720 = streamData.videoInfoList.find((v: any) => v.quality === "720p");
            setSelectedQuality(q720 ? "720p" : streamData.videoInfoList[0].quality);
        }
    }, [streamData, selectedQuality]);

    const currentVideo = streamData?.videoInfoList?.find((v: any) => v.quality === selectedQuality) || streamData?.videoInfoList?.[0];

    // Video Loader - handles both MP4 and HLS
    ...
    useEffect(() => {
        if (currentVideo?.videoPath) {
            loadVideo(currentVideo.videoPath);
        }
        return () => {
            if (hlsRef.current) {
                hlsRef.current.destroy();
                ...
                <div className="relative z-10 flex items-center justify-between h-full px-4 max-w-7xl mx-auto pointer-events-auto">
                    ...
                    <div className="text-center flex-1 px-4 min-w-0">
                        ...
                        <p className="text-white/80 text-xs drop-shadow-md">
                            Episode {currentEpisode?.chapterIndex || 1}
                        </p>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="z-[100]">
                                {streamData?.videoInfoList?.map((v: any) => (
                                    <DropdownMenuItem
                                        key={v.quality}
                                        onClick={() => setSelectedQuality(v.quality)}
                                        className={selectedQuality === v.quality ? "text-primary font-semibold" : ""}
                                    >
                                        {v.quality}
                                    </DropdownMenuItem>
                                ))}
                ...
                <div className="absolute bottom-20 md:bottom-12 left-0 right-0 z-40 pointer-events-none flex justify-center pb-safe-area-bottom">
                    <div className="flex items-center gap-2 md:gap-6 pointer-events-auto bg-black/60 backdrop-blur-md px-3 py-1.5 md:px-6 md:py-3 rounded-full border border-white/10 shadow-lg transition-all scale-90 md:scale-100 origin-bottom">
                        ...
                        <span className="text-white font-medium text-xs md:text-sm tabular-nums min-w-[60px] md:min-w-[80px] text-center">
                            Ep {(currentEpisode?.chapterIndex || 1)} / {totalEpisodes}
                        </span>
